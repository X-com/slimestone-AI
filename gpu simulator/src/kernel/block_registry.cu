...
// Verbatim port of cpp extract's block_registry.cpp buildBlockData() - same id lists, same
// field assignments, just written into a plain array instead of returned by value.
std::array<BlockData, 256> buildBlockRegistryTable() {
    ...
    for (int id : normalCubes) data[static_cast<std::size_t>(id)].isNormalCube = true;
    ...
    for (int id : destroyBlocks) data[static_cast<std::size_t>(id)].pushReaction = PushReaction::Destroy;

    const int blockBlocks[] = {7, 90, 119, 145, 166, 209};
    for (int id : blockBlocks) data[static_cast<std::size_t>(id)].pushReaction = PushReaction::Block;
    ...
    for (int id : tileBlocks) data[static_cast<std::size_t>(id)].hasTileEntity = true;

    const int immovable[] = {7, 90, 119, 120, 137, 166, 209, 210, 211, 255};
    for (int id : immovable) data[static_cast<std::size_t>(id)].hardness = -1.0f;
