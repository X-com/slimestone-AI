"""Auto-provisions a TLS certificate for the flyer-web-visualizer's wss:// live stream, so
wss://localhost:<port> works out of the box for anyone who clones the repo - no manual
mkcert/openssl commands to type by hand. See flyer-web-visualizer/docs/training-integration.md.
"""
...
_DEFAULT_CERT_DIR = Path(__file__).resolve().parents[2] / "data" / "dev-certs"
...
def tls_hint(host: str, port: int) -> str:
    ...
    connection, so an untrusted (self-signed openssl-fallback, or mkcert before `-install` has
    run for that browser) cert just makes every wss:// connect attempt fail silently until the
    browser is told to trust it by visiting the https:// origin directly at least once."""
    return (
