...
struct Result {
    std::int64_t id = 0;
    bool ok = false;
    // Burn-in = the one-off transient before the machine settles into its repeating loop.
    // burninTicks = number of ticks from launch until the loop's first state is reached.
    int burninTicks = 0;
    // burninShift = net (x,y,z) translation the machine underwent during that transient.
    BlockPos burninShift;
    // Cycle = the repeating loop the machine settles into (found when a world-state hash repeats).
    // cycleTicks = the loop period (ticks per loop). Meaningless when timeout.
    int cycleTicks = 0;
    // cycleShift = net (x,y,z) translation per loop. Zero => the machine stopped or is in a
    // stationary loop; nonzero => a shifting loop (a genuine flying machine). Python infers
    // working/not-working from this.
    BlockPos cycleShift;
    // timeout = true if the tick limit was reached before any world-state hash repeated (no loop).
    bool timeout = false;
...
private:
    struct ShiftCycle {
        int start = 0;   // tick the loop's first state occurred = burn-in length
        int end = 0;     // tick that first state recurred
        int period = 0;  // end - start = loop length
        BlockPos shift;       // net translation across one loop (anchor delta start->end)
        BlockPos burninShift; // net translation from launch (tick 0) to loop entry (tick start)
    };
    ...
    // Deletes every BLOCK_TRIGGER block via the normal break path, kicking the machine into motion.
    // Returns the number of trigger blocks removed. Replaces the old redstone-pulse trigger().
    int fireTriggerBlocks();
    void tickWorld();
