...
std::string Result::toJson() const {
    ...
    out << '{'
        << "\"id\":" << id
        << ",\"ok\":" << (ok ? "true" : "false")
        << ",\"burnin_ticks\":" << burninTicks
        << ",\"burnin_shift\":{\"x\":" << burninShift.x
        << ",\"y\":" << burninShift.y
        << ",\"z\":" << burninShift.z << '}'
        << ",\"cycle_ticks\":" << cycleTicks
        << ",\"cycle_shift\":{\"x\":" << cycleShift.x
        << ",\"y\":" << cycleShift.y
        << ",\"z\":" << cycleShift.z << '}'
...
Result Simulator::simulate(const Candidate& candidate) {
    ...
    try {
        ...
        // Kick-start: delete the dedicated trigger block(s); the resulting break/neighbor update is
        // what starts the machine. The state after this (still tick 0) is what detectShiftCycle seeds.
        fireTriggerBlocks();
        ...
        result.ok = true;
        if (found == nullptr) {
            // Tick limit reached with no repeated world-state hash: the whole run was burn-in that
            // never closed into a loop. cycle_* stay zero; timeout signals the values are unusable.
            result.timeout = true;
            result.burninTicks = maxTicks;
        } else {
            // A previous world-state hash was reached: the machine stopped, is in a stationary loop
            // (zero cycle_shift), or a shifting loop (nonzero cycle_shift). Report the burn-in
            // transient and the loop; Python infers working/not-working from cycle_shift.
            result.timeout = false;
            result.burninTicks = found->start;
            result.burninShift = found->burninShift;
            result.cycleTicks = found->period;
            result.cycleShift = found->shift;
        }
...
int Simulator::fireTriggerBlocks() {
    // Collect trigger-block positions first, then break them: setBlockToAir() mutates world_.blocks
    // (and its entries() list), so we must not delete while iterating that list.
    std::vector<BlockPos> triggers;
    for (const auto& e : world_.blocks.entries()) {
        if (blockId(e.state) == BLOCK_TRIGGER) {
            triggers.push_back(unpackPos(e.key));
        }
    }
    for (const BlockPos& pos : triggers) {
        // Break it like any block: setBlockToAir() -> setBlockState(pos, 0, 3) notifies neighbors and
        // observers, which is the update cascade that kicks the machine into motion.
        setBlockToAir(pos);
    }
...
Simulator::ShiftCycle* Simulator::detectShiftCycle(int maxTicks, ShiftCycle& out) {
    ...
    const BlockPos seedAnchor = anchor;  // anchor of the tick-0 (post-trigger-removal) state
    ...
    for (int tick = 1; tick <= maxTicks; ++tick) {
        ...
        if (it != seen.end()) {
            // The world state (all blocks + events, translation-invariant) has returned to a
            // previously seen one, so from here it loops forever. The net translation between the
            // two identical states is the anchor delta: zero => stopped/stationary loop, nonzero
            // => shifting loop (a flying machine). Return on the first repeat regardless of shift.
            const BlockPos& loopAnchor = it->second.anchor;  // anchor at the loop's first occurrence
            out.start = it->second.tick;  // burn-in length (ticks before the loop began)
            out.end = tick;
            out.period = tick - it->second.tick;  // loop length
            out.shift = BlockPos{anchor.x - loopAnchor.x, anchor.y - loopAnchor.y, anchor.z - loopAnchor.z};
            out.burninShift = BlockPos{loopAnchor.x - seedAnchor.x, loopAnchor.y - seedAnchor.y, loopAnchor.z - seedAnchor.z};
            return &out;
        }
