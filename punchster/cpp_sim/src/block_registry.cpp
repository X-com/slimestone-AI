...
std::array<BlockData, 256> buildBlockData() {
    ...
    // Dedicated trigger block: a plain solid cube while present; the simulator breaks it at tick 0 to
    // start the machine (see Simulator::simulate). Left as a normal cube so removing it fires the same
    // neighbor/observer cascade a broken block would.
    data[BLOCK_TRIGGER].isNormalCube = true;
